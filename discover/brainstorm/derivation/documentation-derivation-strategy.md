...
## 🔧 Claude Code Skills for Derivation
...
---
name: loom-derive
description: Derive documentation from source documents
arguments:
  - name: from
    description: Source document type (e.g., user-stories, domain-model)
    required: true
  - name: to
    description: Target document type (e.g., acceptance-criteria, tests)
    required: true
  - name: auto-approve
    description: Skip human approval for this derivation
    required: false
---

# Loom Derivation Skill

Automatically derive documentation based on derivation rules.

## Workflow:

1. **Read source document(s)**
2. **Apply derivation rules** (see Documentation Derivation Strategy)
3. **Generate target document**
4. **Validate output** (structure, traceability, completeness)
5. **Show diff preview**
6. **Ask for approval** (unless --auto-approve)
7. **Write target document**
8. **Trigger downstream derivations** (if applicable)

## Example Usage:

User: /loom-derive --from user-stories --to acceptance-criteria

Claude:
  1. Reads user-stories.md
  2. Finds US-RFQ-001
  3. Generates AC-RFQ-001-1, AC-RFQ-001-2, AC-RFQ-001-3
  4. Shows diff preview
  5. Asks: "Approve 3 new acceptance criteria for US-RFQ-001?"

User: yes

Claude:
  6. Writes acceptance-criteria.md
  7. Runs validation
  8. Reports: "✓ Derived 3 acceptance criteria from 1 user story"
```
